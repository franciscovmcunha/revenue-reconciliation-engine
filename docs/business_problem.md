...
## Three numbers that should agree, and usually don't exactly

For any given day, a hotel can point to three different records of "how much revenue
came in": what the PMS billed, what the cash register took in, and what the card
network settled. (Municipal tourist tax isn't a fourth number here — a cash TTM
payment is visible inside the cash register's own data; paid any other way, it's
folded into the PMS invoice with nothing separate to reconcile — see
[`source_system_analysis.md`](source_system_analysis.md).) When these three are
reconciled manually — spreadsheet by spreadsheet, month by month — small
discrepancies get absorbed into "close enough" long before anyone can say precisely
why two numbers don't match.
...
Not a single dashboard that shows the three totals side by side — that's what a
...
three independently-produced, inconsistently-formatted sources into one comparable shape
