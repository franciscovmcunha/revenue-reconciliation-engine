...
CANONICAL_MONTH = {1: "Janeiro", 2: "Fevereiro", 3: "Março", 4: "Abril", 5: "Maio"}
MONTH_NUMBER = {v: k for k, v in CANONICAL_MONTH.items()}


def run(file_path: str, ingestion_run_id: str) -> None:
    # macOS stores accented filenames in NFD (decomposed) form — "ç" as "c" +
    # a separate combining cedilla — while this literal "Março" in source is
    # NFC (composed). Same glyph, different bytes; normalize before matching.
    ...
    print(f"cash_register ({file_path}): {len(result.valid_rows)} processed, {len(result.rejected_rows)} rejected")